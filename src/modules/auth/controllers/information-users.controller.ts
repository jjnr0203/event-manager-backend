...
export class InformationUsersController {
    constructor(private readonly informationUsersService:InformationUsersService){}
  ...
  @Patch(':id')
  async update(@Param('id') id: string, @Body() payload: UpdateInformationUserDto) {
    const informationUser = await this.informationUsersService.update(id, payload);
    return informationUser;
  }
